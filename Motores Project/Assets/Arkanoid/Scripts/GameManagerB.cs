...
public class GameManagerB : MonoBehaviour
{
    ...
    public void SpawnNovoJogador()
    {
        GameObject playerGameObj = Instantiate(playerPrefab, playerSpawnPoint.position, Quaternion.identity);
        GameObject ballObj = Instantiate(ballPrefab, ballSpawnPoint.position, Quaternion.identity);

        playerAtual = playerGameObj.GetComponent<PlayerB>();
    ...
    public void SubtrairVida()
    {
        ...
        {
            Invoke(nameof(SpawnNovoJogador),2);
        }
    }
